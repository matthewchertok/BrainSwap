...
The separate access-request flow also starts with Google OAuth. If the primary verified provider is Google and no active exact-email invitation can be claimed, the Worker sends only the normalized Auth email and the fixed phrase “is requesting access to BrainSwap” to the configured operator through Resend, then signs the requester out. It does not create a membership or select an organization. A stable hashed Resend idempotency key suppresses duplicate delivery for the provider's documented 24-hour key-retention window.
...
The initial audit found a critical SQL three-valued-logic flaw: several privileged functions compared fields of a possibly NULL actor with `<>`, allowing the `IF` check to fall through. One path could create an administrator invitation in another organization. The additive hardening migration replaces these checks. The later workflow migration preserves those authorization rules while simplifying product fields and adding narrowly authorized invitation deletion, result correction, atomic draft publication, and profile-photo paths. On 2026-09-03, a clean reset applied all three migrations, database lint passed, and the expanded pgTAP suite passed 264/264. Hosted policy/configuration and authenticated browser verification remain required before a pilot.

## Content visibility

Job title and task summary are organization-visible after publication. Drafts are limited to requester/admin. Every published job uses the same sealed mode: prompt, shared-chat link, file metadata, submissions, and revision instructions must not be returned merely because a member can see the listing.
...
A result author may correct the latest submitted result in place only while the job is still `submitted` and the requester has not made a workflow response. The first accept, revision request, cancellation, reopen, or follow-up action locks the result and any unfinished file cleanup. The database locks the job and submission rows so requester action and correction races have a deterministic winner. Direct table writes remain denied.

## Files and Storage

`job-files` is configured as a private bucket with a 25 MiB object limit. The application-level reservation limit is 100 MiB per job, including pending reservations. Supported formats are PDF, plain text/Markdown/CSV/JSON, PNG/JPEG/WebP, and non-macro Office Open XML. HTML, SVG, scripts, executables, generic binary uploads, and macro-enabled Office files are outside the allowlist. `profile-photos` is a separate private bucket limited to JPEG/PNG/WebP and 5 MiB; ready photos are organization-visible, while only the owner may reserve, replace, or remove one.
...
Logs and audit metadata must exclude prompts, responses, revision instructions, credentials, tokens, OAuth codes, sensitive URLs, filenames, and file contents. There is no application analytics/tracking subsystem; the checked-in Worker configuration also disables Wrangler usage metrics and dependency instrumentation. Any optional webhook is server-only, metadata-only, and best effort after the core transaction; hosted endpoint ownership and behavior require manual verification.

Resend is a narrowly scoped external processor for access-request email. The API key, recipient, and sender are server-only Worker settings. The email is plain text, contains the requester's verified email and no task data, and has no link or tracking content. Cloudflare and application logs must not record the address or message body. Resend necessarily processes and retains its transactional-email record under the operator's Resend account, so the lab must accept that privacy boundary before enabling the feature.

## Governance and residual risk

Organization administrators can inspect organization content. Compromised Google, Supabase, Cloudflare, Resend, administrator, helper, or AI-provider accounts remain material risks. A person with many Google accounts can still generate distinct request emails; the control is duplicate suppression per verified address, not a global anti-abuse system. BrainSwap does not virus-scan files, control a helper's manual provider use, guarantee provider deletion, or validate scientific correctness. Operators must define retention, backups, incident contacts, approved providers, collaborator permissions, and institutional data classification before a pilot.
