...
SvelteKit origin checks remain enabled. Security headers include a SvelteKit-generated CSP, `frame-ancestors 'none'`, `object-src 'none'`, nosniff, `Referrer-Policy: same-origin`, a restrictive Permissions Policy, frame denial, and `no-store` for authenticated/authentication responses. `same-origin` sends no referrer to external sites while preserving the real `Origin` on native same-origin form POSTs so SvelteKit can enforce its production CSRF check. These headers still require hosted-response verification after deployment.
