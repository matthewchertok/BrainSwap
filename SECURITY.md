...
SvelteKit origin checks remain enabled. Security headers include a SvelteKit-generated CSP, `frame-ancestors 'none'`, `object-src 'none'`, nosniff, no-referrer, a restrictive Permissions Policy, frame denial, and `no-store` for authenticated/authentication responses. These headers still require hosted-response verification after deployment.
