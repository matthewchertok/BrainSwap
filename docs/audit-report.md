...
- The application test file contains 17 passing Vitest cases covering `/app`-only return paths, protocol-relative/external/control/backslash cases, credential-bearing and malformed-authority URLs, job/form validation, significant-whitespace preservation, filenames/file allowlists, size totals, organization selection, aligned HTTP/document referrer policies, the bounded OAuth CSP form-navigation allowlist, other security headers, status/control display, prompts, and metadata-sized notifications.
...
| `npm test`                                       | PASS                         | Vitest reported 1 file and 17 tests passed after the bounded OAuth CSP form-navigation regression was added                                                                           |
...
| Hosted OAuth/Storage/authenticated browser tests | FAIL / IN PROGRESS           | Worker POST now returns its OAuth 303, but the deployed `'self'`-only `form-action` policy blocked the cross-origin redirect chain in browsers; OAuth must be retested after this fix |
