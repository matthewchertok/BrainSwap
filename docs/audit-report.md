...
- The application test file contains 24 passing Vitest cases. In addition to return-path, validation, organization-selection, CSP/header, display, prompt, and notification coverage, the new cases verify primary confirmed-Google identity derivation, the exact minimal access-request body, normalized address, hashed stable idempotency key, server-only authenticated Resend request shape, pre-network rejection of invalid addresses, pending-tab accessibility markup, versioned login copy, POST-only request markup, and the requested accent token.
...
| `npm test`                                       | PASS                         | Vitest reported 1 file and 24 tests passed, including five access-request email and two UI-feedback contract regressions                                                                                                               |
...
| `npm run deploy:check`                           | PASS                         | Wrangler 4.127.1 parsed the explicit Worker/assets configuration and completed a non-mutating upload dry run without auto-configuration or runtime warnings                                                                            |
