...
- The application test file contains 16 passing Vitest cases covering `/app`-only return paths, protocol-relative/external/control/backslash cases, credential-bearing and malformed-authority URLs, job/form validation, significant-whitespace preservation, filenames/file allowlists, size totals, organization selection, aligned HTTP/document referrer policies and other security headers, status/control display, prompts, and metadata-sized notifications.
...
| `npm test`                                       | PASS                         | Vitest reported 1 file and 16 tests passed after the document-level referrer-policy regression was added                                                                           |
...
| Hosted OAuth/Storage/authenticated browser tests | FAIL / IN PROGRESS           | Worker deployment succeeded, but login POST failed closed in Safari and Chrome because a conflicting document meta referrer policy remained; OAuth must be retested after this fix |
