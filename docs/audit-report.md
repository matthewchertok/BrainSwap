...
- The first hosted login POST exposed an incompatibility between `Referrer-Policy: no-referrer` and SvelteKit's production CSRF origin check: standards-compliant native form navigations sent `Origin: null`, so both Safari and Chrome received a fail-closed 403 before OAuth began. The corrected `same-origin` policy still withholds referrers from external sites while preserving a verifiable Origin for same-origin mutations. A focused header regression test was added; redeployed OAuth acceptance remains required.
...
- CSRF-compatible `same-origin` referrer policy without cross-site referrer disclosure; and
...
- The application test file contains 15 passing Vitest cases covering `/app`-only return paths, protocol-relative/external/control/backslash cases, credential-bearing and malformed-authority URLs, job/form validation, significant-whitespace preservation, filenames/file allowlists, size totals, organization selection, security headers, status/control display, prompts, and metadata-sized notifications.
